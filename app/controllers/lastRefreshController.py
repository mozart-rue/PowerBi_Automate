...
import app.models.lastRefreshModel as dataModel
...
try:
    ...
    print(f'LastRefreshController :: query select na base de dados :: ERRO => {err}')
    connected = False
...
if connected == True:
    ...
    # Faz o Delete antes de inserir novos dados
    dataModel.Delete_LastRefresh()

    # Percore a lista de itens retornados do DB e faz o request na API
    for datasetId, name in db_return:

        url = f'https://api.powerbi.com/v1.0/myorg/datasets/{datasetId}/refreshes?$top=1'
        ...
        if data:
            ...
            for i in data:
                ...
                try:
                    dataModel.Insert_LastRefresh(insert) 
                except Exception as err:
                    print(f'LastRefreshController :: Insert dados no banco :: ERROR => {err}')

        else:
            print(f"{datasetId} Name: {name} não posui historico de atualizações ainda")

else:
    print('LastRefreshController :: Erro => Não foi possivel executar a query no banco de dados')
